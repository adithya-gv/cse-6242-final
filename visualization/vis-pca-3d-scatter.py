...
# Perform PCA on the scaled features to reduce to 3 dimensions
pca = PCA(n_components=3)
df_scaled[['pca_x', 'pca_y', 'pca_z']] = pca.fit_transform(df_scaled[features])
...
# Layout of the app
app.layout = html.Div([
    html.H1("Game Similarity Visualization (3D)", style={"text-align": "center", "margin-bottom": "20px"}),
    dcc.Dropdown(
        id='game-dropdown',
...
def update_scatter(selected_game):
    ...
    # Create 3D scatter plot using PCA components
    fig = px.scatter_3d(
        top_100_games,
        x='pca_x',
        y='pca_y',
        ...
        title="3D PCA Scatter Plot of Clusters (Top 100 Games)"
    )
